...
class SubtitleSettingsOverlay extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final currentSize =
        const [16.0, 22.0, 28.0, 34.0].contains(rawSize) ? rawSize : 22.0;
    final currentColor = const [
      0xFFFFFFFF,
      0xFFFFF176,
      0xFF00E5FF,
      0xFF69F0AE,
    ...
    final currentBgColor = const [
      ...
      0xFF000000
        ...
        ? rawBgColor
        : 0xD9000000;
    final currentOutline =
        const [0.0, 0.6, 1.2, 2.2].contains(rawOutline) ? rawOutline : 1.2;
    final currentFont = const [
      'System',
      'Georgia',
      'Times New Roman',
      'Menlo',
                      ...
                      children: [
                        ...
                        () {
                          final availableOptions =
                              ref.watch(availableSubtitlesProvider);
                          final currentPrimary =
                              ref.watch(selectedSubtitleProvider);
                          final currentSecondary =
                              ref.watch(selectedSecondarySubtitleProvider);

                          final primaryId = availableOptions
                                  .any((o) => o.id == currentPrimary?.id)
                              ? currentPrimary?.id ?? 'none'
                              : 'none';
                          final secondaryId = availableOptions
                                  .any((o) => o.id == currentSecondary?.id)
                          ...
                          return Column(
                            ...
                            children: [
                              _buildSettingDropdown<String>(
                                ...
                                onChanged: (id) {
                                  if (id != null) {
                                    final selectedOpt = availableOptions
                                ...
                                onChanged: (id) {
                                  if (id != null) {
                                    final selectedOpt = availableOptions
                                        .firstWhere((o) => o.id == id);
                                    ref
                                        .read(selectedSecondarySubtitleProvider
                                            .notifier)
                                        .state = selectedOpt;
                                  }
                                },
                        ...
                        _buildSettingDropdown<double>(
                          ...
                          items: [
                            DropdownMenuItem(
                                value: 16.0,
                                child: Text(isPersian ? 'کوچک' : 'Small')),
                            DropdownMenuItem(
                                value: 22.0,
                                child: Text(isPersian ? 'متوسط' : 'Medium')),
                            DropdownMenuItem(
                                value: 28.0,
                                child: Text(isPersian ? 'بزرگ' : 'Large')),
                            DropdownMenuItem(
                                value: 34.0,
                                child: Text(
                                    isPersian ? 'خیلی بزرگ' : 'Extra Large')),
                          ],
                          onChanged: (val) {
                            if (val != null) {
                              ref.read(subtitleSizeProvider.notifier).state =
                                  val;
                        ...
                        _buildSettingDropdown<int>(
                          ...
                          items: [
                            DropdownMenuItem(
                                value: 0xFFFFFFFF,
                                child: Text(isPersian ? 'سفید' : 'White')),
                            DropdownMenuItem(
                                value: 0xFFFFF176,
                                child: Text(isPersian ? 'زرد' : 'Yellow')),
                            DropdownMenuItem(
                                value: 0xFF00E5FF,
                                child: Text(isPersian ? 'فیروزه‌ای' : 'Cyan')),
                            DropdownMenuItem(
                                value: 0xFF69F0AE,
                          ...
                          onChanged: (val) {
                            if (val != null) {
                              ref.read(subtitleColorProvider.notifier).state =
                                  val;
                        ...
                        _buildSettingDropdown<int>(
                          ...
                          items: [
                            ...
                            DropdownMenuItem(
                                value: 0xD9000000,
                                child: Text(isPersian ? 'تیره' : 'Dark')),
                            DropdownMenuItem(
                                value: 0xFF000000,
                                child: Text(
                                    isPersian ? 'مشکی کامل' : 'Solid (Black)')),
                          ],
                          onChanged: (val) {
                            if (val != null) {
                              ref.read(subtitleBgColorProvider.notifier).state =
                                  val;
                        ...
                        _buildSettingDropdown<double>(
                          label:
                              isPersian ? 'ضخامت حاشیه' : 'Outline Thickness',
                          value: currentOutline,
                          items: [
                            DropdownMenuItem(
                                value: 0.0,
                                child: Text(isPersian ? 'بدون حاشیه' : 'None')),
                            DropdownMenuItem(
                                value: 0.6,
                                child: Text(isPersian ? 'باریک' : 'Thin')),
                            DropdownMenuItem(
                                value: 1.2,
                                child: Text(isPersian ? 'متوسط' : 'Medium')),
                            DropdownMenuItem(
                                value: 2.2,
                                child: Text(isPersian ? 'پهن' : 'Thick')),
                          ],
                          onChanged: (val) {
                            if (val != null) {
                              ref
                                  .read(subtitleOutlineWidthProvider.notifier)
                                  .state = val;
                        ...
                        _buildSettingDropdown<String>(
                          ...
                          items: const [
                            ...
                            DropdownMenuItem(
                                ...
                                child: Text('Times New Roman (Serif)')),
                            DropdownMenuItem(
                                value: 'Menlo',
                                child: Text('Menlo (Monospace)')),
                            DropdownMenuItem(
                                value: 'Courier New',
                                child: Text('Courier New (Monospace)')),
                            DropdownMenuItem(
                                value: 'Helvetica Neue',
                                child: Text('Helvetica Neue (Sans-serif)')),
                            DropdownMenuItem(
                                value: 'Avenir',
                                child: Text('Avenir (Sans-serif)')),
                          ],
                          onChanged: (val) {
                            if (val != null) {
                              ref
                                  .read(subtitleFontFamilyProvider.notifier)
                                  .state = val;
                          ...
                          child: Row(
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      isPersian
                                          ? 'پریش بر اساس خطوط زیرنویس به جای ۱۰ ثانیه'
                              ...
                              Switch.adaptive(
                                ...
                                onChanged: (val) {
                                  ref
                                      .read(smartSubtitleSeekProvider.notifier)
                                      .state = val;
