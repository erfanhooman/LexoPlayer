...
const Map<String, String> nounLemmas = {
  ...
  'feet': 'foot', 'teeth': 'tooth', 'lights': 'light',
  'lives': 'life', 'wives': 'wife', 'knives': 'knife',
  'thieves': 'thief', 'leaves': 'leaf', 'wolves': 'wolf',
  'halves': 'half', 'calves': 'calf', 'shelves': 'shelf',
  'classes': 'class', 'buses': 'bus', 'dishes': 'dish',
  'watches': 'watch', 'boxes': 'box', 'foxes': 'fox',
  'churches': 'church', 'branches': 'branch', 'sandwiches': 'sandwich',
  'potatoes': 'potato', 'tomatoes': 'tomato', 'heroes': 'hero',
  'torpedoes': 'torpedo', 'volcanoes': 'volcano',
  'analyses': 'analysis', 'bases': 'basis', 'crises': 'crisis',
  'diagnoses': 'diagnosis', 'oases': 'oasis', 'theses': 'thesis',
  'phenomena': 'phenomenon', 'criteria': 'criterion', 'data': 'datum',
  'alumni': 'alumnus', 'fungi': 'fungus', 'nuclei': 'nucleus',
  'stimuli': 'stimulus', 'syllabi': 'syllabus', 'termini': 'terminus',
  'virtues': 'virtue', 'values': 'value',
  'things': 'thing', 'times': 'time', 'days': 'day',
  'years': 'year', 'worlds': 'world',
  'ways': 'way', 'works': 'work', 'words': 'word',
  'places': 'place', 'cases': 'case', 'problems': 'problem',
  'points': 'point', 'groups': 'group', 'companies': 'company',
  'numbers': 'number', 'systems': 'system', 'programs': 'program',
  'questions': 'question', 'government': 'government',
  'countries': 'country', 'stories': 'story', 'facts': 'fact',
  'months': 'month', 'lot': 'lot', 'bits': 'bit',
  'cities': 'city', 'members': 'member',
  'players': 'player', 'teams': 'team', 'games': 'game',
  'schools': 'school', 'students': 'student', 'teachers': 'teacher',
  'books': 'book', 'movies': 'movie', 'songs': 'song',
  'ideas': 'idea', 'parts': 'part',
  'hands': 'hand', 'eyes': 'eye', 'heads': 'head',
  'faces': 'face', 'bodies': 'body', 'minds': 'mind',
  'hearts': 'heart', 'doors': 'door', 'windows': 'window',
  'tables': 'table', 'chairs': 'chair', 'rooms': 'room',
  'houses': 'house', 'cars': 'car', 'trees': 'tree',
  'flowers': 'flower', 'dogs': 'dog', 'cats': 'cat',
  'birds': 'bird', 'fish': 'fish', 'horses': 'horse',
};
...
class SimplePosTagger {
  ...
  static final Set<String> _determiners = {
    ...
    'each', 'all', 'both', 'few', 'more', 'most', 'much', 'many',
    'such', 'what', 'which', 'whose', 'whatever', 'whichever',
  };
  ...
  static final Set<String> _prepositions = {
    ...
    'behind', 'beside', 'beyond', 'against', 'along', 'among', 'around',
    'before', 'behind', 'below', 'beneath', 'beside', 'besides',
    'beyond', 'concerning', 'despite', 'during', 'except', 'excluding',
    'following', 'inside', 'into', 'like', 'minus', 'near', 'onto',
    'outside', 'over', 'past', 'pending', 'per', 'plus', 'regarding',
    'round', 'since', 'through', 'throughout', 'till', 'toward',
    'towards', 'under', 'underneath', 'until', 'unto', 'upon', 'using',
    'via', 'within', 'without',
  };
  ...
  static final Set<String> _conjunctions = {
    ...
    'after', 'before', 'when', 'where', 'how', 'if', 'whether',
    'than', 'though', 'even', 'once', 'as', 'whereas', 'whereby',
  };
  ...
  static final Set<String> _adverbs = {
    'not', 'also', 'very', 'often', 'never', 'always', 'sometimes',
    'really', 'already', 'just', 'still', 'even', 'only', 'almost',
    'quite', 'too', 'here', 'there', 'now', 'then', 'soon', 'today',
    'yesterday', 'tomorrow', 'ago', 'later', 'earlier', 'probably',
    'certainly', 'definitely', 'perhaps', 'maybe', 'actually', 'basically',
    'simply', 'quickly', 'slowly', 'easily', 'hardly', 'barely',
    'recently', 'finally', 'suddenly', 'immediately', 'eventually',
    'usually', 'generally', 'normally', 'typically', 'apparently',
    'obviously', 'clearly', 'simply', 'highly', 'extremely', 'really',
    'enough', 'rather', 'pretty', 'fairly', 'mostly', 'entirely',
    'completely', 'absolutely', 'totally', 'utterly',
  };

  static final Set<String> _interjections = {
    'oh', 'wow', 'hey', 'hi', 'hello', 'goodbye', 'bye',
    'yes', 'no', 'okay', 'ok', 'well', 'alas', 'ouch',
    'hmm', 'huh', 'ah', 'ugh', 'phew', 'oops',
  };
  ...
  static String tagWord(String word, {bool atSentenceStart = false, String? nextWord}) {
...
class EngineTokenizer {
  ...
  List<EngineTokenInfo> filterTargetTokens(
    ...
    return tokens
        .where((t) =>
            t.startChar >= targetStartChar && t.endChar <= targetEndChar)
        .toList();
  }
    ...
    if (pos == 'VERB' || pos == 'AUX') {
      ...
      if (word.endsWith('ing') && word.length > 4) {
        ...
        if (stem.isNotEmpty) {
          ...
          // -ying: dying -> die
          if (stem.endsWith('y')) return '${stem.substring(0, stem.length - 1)}ie';
    ...
    if (pos == 'NOUN' || pos == 'PROPN') {
      ...
      if (word.endsWith('es') && word.length > 3) {
        ...
        if (stem.endsWith('s') || stem.endsWith('x') || stem.endsWith('z') ||
            stem.endsWith('sh') || stem.endsWith('ch')) {
      ...
      if (word.endsWith('s') && !word.endsWith('ss') &&
          !word.endsWith('us') && !word.endsWith('is') && word.length > 3) {
        return word.substring(0, word.length - 1);
      }
    }
